...
class TD3(object):
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with torch.no_grad():
            ...
            noise = torch.max(torch.min(torch.randn_like(action) * self.policy_noise, self.noise_clip), -self.noise_clip)

            next_action = torch.max(torch.min(self.actor_target(next_state) + noise, self.max_action), self.min_action)
